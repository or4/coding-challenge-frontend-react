...
describe('Check saga for IncidentsRequest', () => {
    const defaultIncidentRequestOptions: IIncidentRequestOptions = {
        incidentType: 'theft',
        proximity: 'Berlin',
        proximitySquare: 100,
        perPage: 10,
    ...
    it('should return success action with options', () => {
        ...
        expect(generator.next().value).toEqual(call(api.get, '/incidents', defaultIncidentRequestOptions));

        const incidents: IIncident[] = [];
        const response = { data: { incidents }, status: 200 };
        // @ts-ignore
        expect(generator.next(response).value).toEqual(put(new IncidentsRequestSuccess(transform(incidents))));
        expect(generator.next().done).toEqual(true);
    });
    ...
    it('should return success action with incidents and only need fields', () => {
        ...
        // @ts-ignore
        expect(generator.next(response).value).toEqual(put(new IncidentsRequestSuccess(expected)));
        expect(generator.next().done).toEqual(true);
    });
